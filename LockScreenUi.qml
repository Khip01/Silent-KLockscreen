...
Item {
    ...
    // ── FontLoader: load Red Hat Display family from local fonts/ ──
    SilentFonts {}
    ...
    // ── Authenticator connections (API: authenticator.respond) ───────────
    Connections {
        target: authenticator
        function onFailed(kind) {
            if (kind != 0) { // non-interactive authenticators
                return;
            }
    ...
    // ── Session management (Suspend → root.suspendToRam,
    //                        Hibernate → root.suspendToDisk,
    //                        Reboot/Shutdown → sessionManagement.*) ──
    SessionManagement {
    ...
    MouseArea {
        ...
        // ── SilentSDDM-style Clock (Red Hat Display) ────────────────────
        DropShadow {
            id: clockShadow
            anchors.fill: silentClock
            source: silentClock
        ...
        Item {
            ...
            width: silentClockColumn.width
            height: silentClockColumn.height

            ColumnLayout {
                id: silentClockColumn
                spacing: SilentConfig.dateMarginTop

                Text {
                    id: silentTime
                    visible: SilentConfig.clockDisplay
                    font.pixelSize: SilentConfig.clockFontSize * SilentConfig.generalScale
                    font.weight: SilentConfig.clockFontWeight
                    font.family: SilentConfig.clockFontFamily
                    color: SilentConfig.clockColor
                    Layout.alignment: Qt.AlignHCenter

                    function updateTime() {
                        text = new Date().toLocaleString(Qt.locale(SilentConfig.dateLocale), SilentConfig.clockFormat);
                    }
                }

                Text {
                    id: silentDate
                    visible: SilentConfig.dateDisplay
                    font.pixelSize: SilentConfig.dateFontSize * SilentConfig.generalScale
                    font.family: SilentConfig.dateFontFamily
                    font.weight: SilentConfig.dateFontWeight
                    color: SilentConfig.dateColor
                    Layout.alignment: Qt.AlignHCenter

                    function updateDate() {
                        text = new Date().toLocaleString(Qt.locale(SilentConfig.dateLocale), SilentConfig.dateFormat);
                    }
                }
                ...
                Timer {
                    ...
                    running: true
                    onTriggered: {
                        silentTime.updateTime();
                        silentDate.updateDate();
                    }
                }
        ...
        StackView {
            ...
            // not implicit — otherwise items still get processed for the scenegraph
            visible: opacity > 0
            ...
            initialItem: MainBlock {
                ...
                // API: authenticator.respond(password)
        ...
        // ── Footer toolbar ─────────────────────────────────────────────
        // Note: Containment masks stretch clickable area buttons to screen
        // edges (Fitts's law).
        RowLayout {
            id: footer
            anchors {
